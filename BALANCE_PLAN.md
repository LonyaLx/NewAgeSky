...
## 万象合金炉进度重做
- 删除 Neo Eco 毕业任务赠送首台万象合金炉的奖励，首台机器与复制机均改为跨模组制造。
- 九阶无用合金接入 IE、Mekanism、AE2、AdvancedAE、Ender IO、工业先锋与天工终局材料。
- T1-T4 使用原版工作台，T5-T9 使用 Avaritia Tier 合成台；不使用自定义合金炉机器配方和机械动力材料。
- 万象合金炉继续兼容 Create 配方转换，但万象物品本身不使用 Create 材料。
- 工作台与 Avaritia Tier 配方全部使用左右镜像对称图案。
- 修复万象平衡脚本中 `recipe` 变量重名；此前该异常会导致脚本不加载并回退旧难度。
- 齿轮与玻璃使用金属模具、对应处理器和跨模组合金；线圈使用上一阶齿轮/玻璃，在原版工作台制作。
- 线圈单次产出 16 → 2；单方块炉、多方块核心、模具中心、ME 样板总成、被动合成仓和样板转换器全部改贵。
- 新增「万象合金炉终局链」章节，共 31 个任务；任务书更新为 23 章、7 组、857 个任务。